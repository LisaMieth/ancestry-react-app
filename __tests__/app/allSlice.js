...
test('allSlice reducer - sets loading status when fetchAll is pending', () => {
  const action = { type: 'all/fetchAll/pending' }
  const state = reducer(initialState, action);
  expect(state).toEqual({ data: [], status: 'loading', error: null })
})

it('allSlice reducer - sets loaded and success status when fetchAll is fulfilled', () => {
...
it('allSlice reducer - sets error and failure status when fetchAll is rejected', () => {
  const action = { type: 'all/fetchAll/rejected', payload: 'Some error' }
  const state = reducer(initialState, action)
  expect(state).toEqual({ data: [], status: 'failed', error: 'Some error' })
})

test('fetchAll - calls apiClient', () => {
  const store = mockStore({ all : { data: [], status: 'idle' } })
  store.dispatch(fetchAll())
  expect(apiClient.get).toHaveBeenCalledTimes(1)
})  

test('fetchAll - dispatches pending & fulfilled action on successful API call', () => {
  ...
  store.dispatch(fetchAll()).then(() => {
    ...
    console.log('PAYLOAD', actions[1]);
    expect(actions[0].type).toEqual('all/fetchAll/pending')
    expect(actions[1].type).toEqual('all/fetchAll/fulfilled')
  })
})

test('fetchAll - dispatches pending & rejected action on unsuccessful API call', () => {
